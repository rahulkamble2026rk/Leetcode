...
public:
    ...
    {   
        ...
        // Repeat until repeated string size >= b
        while (repeated.size() < b.size()) {
            repeated += a;
            ans++;
        }

        // First check without extra repetition
        if (isSubstring(repeated, b)) return ans;

        // Then check with one more repetition (edge case)
        repeated += a;
        if (isSubstring(repeated, b)) return ans + 1;
